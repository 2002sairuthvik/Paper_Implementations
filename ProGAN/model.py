...
#         super().__init__()
#         self.conv = nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding)
#         self.scale = (gain / (in_channels * (kernel_size ** 2))) ** 0.5  # Equalized LR scale
#         self.bias = self.conv.bias
#         self.conv.bias = None
...
#     def fade_in(self, alpha, upscaled, generated):
#         # Blend layer outputs with tanh activation as in original ProGAN paper
#         return torch.tanh(alpha * generated + (1 - alpha) * upscaled)

#     def forward(self, x, alpha, steps):
#         out = self.initial(x)  # 4x4 start
#         if steps == 0:
#             return self.initial_rgb(out)
...
#     def minibatch_std(self, x):
#         batch_std = torch.std(x, dim=0, unbiased=False).mean()
#         batch_std = batch_std.repeat(x.shape[0], 1, x.shape[2], x.shape[3])
...
# if __name__ == "__main__":
#     Z_DIM = 100  # Match latent size to both G and D
#     IN_CHANNELS = 256
...
#         assert z.shape == (1, 3, img_size, img_size)
#         out = critic(z, alpha=0.5, steps=num_steps)
#         print(f"Success! At img size: {img_size}, Discriminator output shape: {out.shape}")
#         assert out.shape == (1, 1)
